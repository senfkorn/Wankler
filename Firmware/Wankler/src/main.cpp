...
LiquidCrystal_I2C lcd(0x27,20,4);  // set the LCD address to 0x27 for a 16 chars and 2 line display // SCL = A5 // SDA = A4
...
//read parameter from eeprom if button is not pressed on startup
  if(digitalRead(A1)){ 
    EEPROM_readAnything(10, Timeout);
    EEPROM_readAnything(20, Threshold);
    EEPROM_readAnything(30, Filter);
  }
...
switch (MenuItem) {
  case 0: //top menu
    showTopMenu();
    break;
  case 1: //zeige Messung
    showMessung();
    break;
  case 2: //parameter menu
    showParameterMenu();
    break;
  ...
  if (Clicked && !Init){
    ...
    case 0: //top menu
      ...
      if (encoder.getPosition()== 1){
        MenuItem = 1; //goto show measurement
      } 
      ...
      break;
    case 1: //measurement
      MenuItem = 0; //goto top menu
      break;
      case 2: //parameter menu
      if (encoder.getPosition()== 0){
        MenuItem = 3; //goto change threshold
      } 
      if (encoder.getPosition()== 1){
        MenuItem = 4; //goto change timeout
      } 
      if (encoder.getPosition()== 2){
        MenuItem = 5;//goto top menu
      }
...
//reset Init value and allow normal operation
if (digitalRead(A1) && Init){
  Init = false;
}
...
void changeTimeout(){
RotaryEncoder::Direction encoderMovement;
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Timeout");
  lcd.setCursor(6,2);
  ...
  do{
  encoderMovement = encoder.getDirection();
    if (encoderMovement !=  RotaryEncoder::Direction::NOROTATION){
      Timeout += (float)encoderMovement * 1000;
      if (Timeout > 20000){
        Threshold = 20000;
...
RotaryEncoder::Direction encoderMovement;
  ...
  encoderMovement = encoder.getDirection();
    if (encoderMovement != RotaryEncoder::Direction::NOROTATION){
      Filter += (float)encoderMovement * 10;
      if (Filter > 32767){
        Filter = 32767;
      }else if(Filter < 1){
        Filter = 1;
